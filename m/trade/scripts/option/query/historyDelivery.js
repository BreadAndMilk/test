 ...
 * 历史交割查询
 */
define("project/scripts/stockOption/stockTrade/query/historyDelivery",function(require,exports,module){
	var appUtils = require("appUtils"),
		...
		_pageId = "#stockOption_stockTrade_query_historyDelivery ";
	/**
	 * 初始化
	 * */
	function init()
	{
		...
		//添加头部
		stock_common.addNavigation(2, 0, 2, 1, "历史交割查询");
...
//			}
	...
	function queryData()
	{
		...
		stock_common.queryDiffTradeDate("0",t,function(data){
			if(data)
			{
				...
				serviceImp.queryHistoryDelivery(param,queryCallBack);
			}
		});
	}
	/**
	...
	function createHtml(element)
	{
		var eleHtml = "";
			...
			eleHtml += "<li><span>成交数量：</span>"+element.occur_amount+"</li>";
			eleHtml += "<li><span>成交价格：</span>"+element.opt_business_price+"</li>";
			eleHtml += "<li><span>成交金额：</span>"+element.business_balance+"</li>";
			eleHtml += "<li><span>后证券余额：</span>"+element.post_amount+"</li>";
			eleHtml += "<li><span>后资金余额:</span>"+element.post_balance+"</li>";
			...
			eleHtml += "<li><span>其他费：</span>"+element.other_fare+"</li>";
			eleHtml +="</ul>";
			eleHtml +="</div>";
		return eleHtml;
	}
	...
	function BindHeaderEvent()
	{
		stock_common.bindEvent($("#stockOption_header #headerName .icon_back"), function(e){
			appUtils.pageInit("stockOption/stockTrade/query/historyDelivery","stockOption/stockTrade/query/mainQuery",{});
			e.stopPropagation();
		});
	}
	/**
	...
	module.exports = historyDelivery;
});