 ...
 * 当日行权指派查询
 */
define("project/scripts/stockOption/stockTrade/query/dayAssign",function(require,exports,module){
	var appUtils = require("appUtils"),
		...
		_pageId = "#stockOption_stockTrade_query_dayAssign ";
	/**
	 * 初始化
	 * */
	function init()
	{
		...
		//添加头部
		stock_common.addNavigation(2, 0, 2, 1, "当日行权指派查询");
	...
	function queryData()
	{
		...
		serviceImp.queryDayAssign(param,queryCallBack);
	}
	/**
	 * 当日行权指派查询数据方法
			...
			if(data.error_no == 0)
			{
				...
				if(results && results.length > 0){
					...
					for(i = 0;i < results.length; i++){
						var data = results[i];
						todayAssignHtml += createHtml(data);
					}
					$(_pageId+"#listContents").append(todayAssignHtml);
				}
				else{
	...
	function createHtml(element)
	{
		var eleHtml = "";
			...
			eleHtml += "<li><span>期权类别：</span>"+element.option_type+"</li>";
			eleHtml += "<li><span>持仓类别：</span>"+element.opthold_type+"</li>";
	...
	function BindHeaderEvent()
	{
		stock_common.bindEvent($("#stockOption_header #headerName .icon_back"), function(e){
			appUtils.pageInit("stockOption/stockTrade/query/dayAssign","stockOption/stockTrade/query/mainQuery",{});
			e.stopPropagation();
		});
	}
	/**
