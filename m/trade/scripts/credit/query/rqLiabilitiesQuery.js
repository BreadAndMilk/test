 ...
 * 信用交易-查询融资融券负债
 */
define('trade/scripts/credit/query/rqLiabilitiesQuery.js', function(require,exports,module){
	var gconfig = $.config;
	var global = gconfig.global;
	var	service_credit = require("service_credit");
	var	financialSecurities = false;
	var	_pageId = "#credit_query_rqLiabilitiesQuery ";
	 ...
	 * 融券负债查询
	 */
	function queryRQLiabilities(type){
		$(_pageId+"#append").html("");
	...
	function initVIScroll(){
		if(!vIscroll._init){
			var config = {
					...
					"downHandle": function() {
						queryRQLiabilities();//下拉获取上一页数据方法
					},
					"upHandle": function() {	
						//上拉获取下一页数据方法
					},
