 ...
 * 信用交易-当日委托
 */
define('trade/scripts/credit/query/creditQueryToday.js', function(require, exports, module) {
    ...
    var _pageId = "#credit_query_creditQueryToday ";
    var userInfo = null;
    var _queryType=null;
    /**
     * 初始化
	...
	function init(){
	    ...
	    _queryType = $.getPageParam("param");
		if(_queryType=="entrust"){
			$(_pageId + ".top_title h3").text("当日委托");
		}else if(_queryType=="deal"){
			$(_pageId + ".top_title h3").text("当日成交");
		}
	...
	function queryEntrust(){
		...
		var param={				
			...
			service_credit.todaywt_query(param,queryEntrustCallback,
				{
				"isLastReq":true,
				"isShowWait":true,
				"isShowOverLay":false,
...
//      eleHtml+="    <dd><em>编&#160;&#160;&#160;号</em>"+element.entrust_no+"</dd>";
        ...
        eleHtml+="    <dd><em>冻结金额</em>"+Number(element.orderfrzamt).toFixed(price_digit)+"</dd>";
        eleHtml+="    <dd><em>成交金额</em>"+Number(element.business_balance).toFixed(price_digit)+"</dd>";
...
//      eleHtml+="    <dd><em>股东代码</em>"+element.stock_account+"</dd>";
	...
	function queryDeal(){
		...
		$(_pageId+ ".no_data").hide();
		service_credit.todaycj_query(param,queryDealCallback,
		    {
			"isLastReq":true,
			"isShowWait":true,
			"isShowOverLay":false,
...
//		});
        ...
        var eleHtml = "";
        eleHtml+="<div class=\"part\">";
        eleHtml+="<span class=\"time\">"+element.business_date+"&nbsp"+element.business_time+"</span>";
        eleHtml+="<h5 class=\""+css+"\">"+element.entrust_type_name+"</h5>";
        eleHtml+="<dl class=\"clearfix\">";
        eleHtml+="	<dd>"+element.stock_name+" <small>"+element.stock_code+"</small></dd>";
        eleHtml+="    <dd><em>成交状态</em>"+element.real_status_name+"</dd>";
        eleHtml+="    <dd><em>委托编号</em>"+element.entrust_no+"</dd>";
        eleHtml+="    <dd><em>成交编号</em>"+element.report_no+"</dd>";
        eleHtml+="    <dd><em>委托数量</em>"+element.entrust_amount+"</dd>";
        eleHtml+="    <dd><em>委托价格</em>"+Number(element.entrust_price).toFixed(price_digit)+"</dd>";
        eleHtml+="    <dd><em>成交数量</em>"+element.business_amount+"</dd>";
        eleHtml+="    <dd><em>成交价格</em>"+Number(element.business_price).toFixed(price_digit)+"</dd>";
        eleHtml+="    <dd><em>成交金额</em>"+element.business_balance+"</dd>";
/*        eleHtml+="    <dd><em>股东代码</em>"+element.stock_account+"</dd>";*/
        eleHtml+="</dl>";
        eleHtml+="</div>";
        return eleHtml;
