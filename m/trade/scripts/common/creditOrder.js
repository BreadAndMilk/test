 ...
 * 交易下单
 */
define('trade/scripts/common/creditOrder.js', function(require, exports, module) {
     ...
     * 数据初始化
     */
	function dataInit(pageId,orderType,market,callback){
		_callback = callback
		_pageId = pageId
		if(keyboard){
			keyboard.keyInit();
...
//		}
	...
	function isBuy(){
		if(_orderType == "3" || _orderType == "5" || _orderType == "7"){
			return true;
		}
		else{
			return false;
...
//		var price_step_const = [1,3,4,5,6,10,11,12,13,14,16,19,20,21,22,23,24,30];//0.001
		var price_step_const = [0,2,7,9,15,17,18,25,26,27,66];//0.01
...
//					$.hidePreloader();
	...
	function callbackFifthOrder(data){
		if(data.errorNo == 0){
			...
			fifthOrder_timer =  window.setTimeout(setFifthOrder, (global.refreshTime || 5000));//定时刷新
		}else{
			if (fifthOrder_timer != null ) {
		        window.clearTimeout(fifthOrder_timer);
		    }
	...
	function queryMaxAmount(stockCode,isMax){
		...
		var param = {
	            ...
	            "entrust_type":entrustType[_orderType]
			};
	...
	function queryHolding(){
		if(_callback && typeof(_callback) == "function"){
			_callback();
			//卖出事件
			$.preBindEvent($(_pageId+".fund_table .ce_table"),"dl", function(e){
				clearBuyMsg();// 清楚数据
