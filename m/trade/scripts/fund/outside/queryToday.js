 ...
 * 场外基金-今日查询
 */
define('trade/scripts/fund/outside/queryToday.js', function(require, exports, module) {
    ...
    var _pageId = "#fund_outside_queryToday ";
    var userInfo = null;
	...
	function bindPageEvent(){
		...
		$.bindEvent($(_pageId + ".top_title .icon_back"), function(e){
			$.pageBack("fund/outside/query","left");
		});
		//页面切换
	...
	function queryEntrust(){
		...
		var param={				
			...
			service_fund.queryTodayTrust(param,queryEntrustCallback,
				{
				"isLastReq":true,
				"isShowWait":true,
				"isShowOverLay":false,
	...
	function queryDeal(){
		...
		$(_pageId+ ".no_data").hide();
		service_fund.queryTodayTrade(param,queryDealCallback,
		    {
			"isLastReq":true,
			"isShowWait":true,
			"isShowOverLay":false,
	...
	function queryDealHTML(element,i){
		...
		var eleHtml = "";
      	eleHtml +='<div class="part">';
        eleHtml +='<div class="title"><span class="tag n1">'+element.business_name+'</span>';
        eleHtml +='<h5>'+element.fund_name+' <small>'+element.fund_code+'</small></h5><span class="stat">'+element.entrust_status_mame+'</span></div><ul>';
        eleHtml +='<li>委托份额  <span>'+Number(element.shares).toFixed(price_digit)+'</span></li>';
        eleHtml +='<li>委托金额 <span>'+Number(element.balance).toFixed(price_digit)+'</span></li>';
        eleHtml +='<li>委托时间 <span>'+element.entrust_time+'</span></li>';
        eleHtml +='</ul></div>';
	 ...
	 */
	function pageBack(){
		$.pageInit("stock/stockQueryToday","stock/stockQuery",{});
	}
