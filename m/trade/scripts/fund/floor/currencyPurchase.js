 ...
 * 普通交易-查询
 */
define('trade/scripts/fund/floor/currencyPurchase.js', function(require, exports, module) {
	var common = require("common");
	var gconfig = $.config; 
    var global = gconfig.global;
    var _pageId = "#fund_floor_currencyPurchase ";
	...
	function vailSubmitOrder(){
		...
		common.iConfirm("下单确定",tipStr,function success(){
			...
			var param = {
			    ...
			    service_fund.getpurchaseBuy(param,function(data){
	...
	function getBuyCallBack(data,param){
		if(data.error_no == 0){
	  	...
	  	}else if(data.error_no == "-30200710") {
		...
		}else if(data.error_no == "-30200711"){
			common.iConfirm("下单提示",data.error_info,function() {
				param.flag = "1";
				service_fund.getpurchaseBuy(param,getBuyCallBack);
			});
		}else{
			$.alert(data.error_info);
		}
