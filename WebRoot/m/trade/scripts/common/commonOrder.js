 ...
 * 交易下单
 */
define('trade/scripts/common/commonOrder.js', function(require, exports, module) {
    ...
    var entrustType = { // 融资融券委托类型
		"3":"6", "4":"7", "5":"0", "6":"0", "7":"7", "8":"6",
	};
	var orderName = { // 委托名称
		"1":"委托买入",
		"2":"委托卖出",
...
//		keyboard.keyInit();
		...
		initHtml();
		_stockCode = null;
		var stockCode = $.getPageParam("stockCode") || $.getPageParam("code");
		// 查询个股行情
		if(stockCode){
...
//		stock_html[key].text(value);
		...
		$.bindEvent($(_pageId + ".trade_form .value_btn a"), function(e){
			if($(this).attr("class") != "current")
			{
				...
				else
				{
					...
					var id = $(_pageId + "#market_list p").attr("id");
					stock_html.market_div.attr("value",id);
					stock_html.total.hide();
					if(!id){
						$(_pageId + "#market_list p").html("<em style='color: #ccc;'>请选择委托方式</em>");
		...
		if(price && validatorUtil.isNumberFloat(price)){
			...
			if(cur_arr.length == 2){
				if(cur_arr[1].length > price_digit){
					$this.val(cur_arr[0]+"."+cur_arr[1].substring(0,price_digit))
				}
			}
		}
	}
	...
	function ListData(data){
		...
		if(stkType == 6 || stkType == 14 || stockVal.substring(0,3) == "131"){
			lotsOf = 10;
		}
		...
		if(stock_info.stock_market == "SH" && market_html_data && market_html_data.SH){
			if(stock_html.market_ul.attr("category") != "SH"){
				$(_pageId + "#market_list p").attr("id","").html("<em style='color: #ccc;'>请选择委托方式</em>");
			}
		...
		if(data.errorNo == 0){
			...
			if(results && results.length > 0){
				if(results.length == 1 && vai1.test(stockCode)){
					...
					$.bindEvent($(_pageId + ".trade_form .select_box ul li"),function(){
...
//					$.hidePreloader();
	...
	function queryMaxAmount(stockCode,isMax){
		...
		if(stock_info.market_price){
			if(stock_html.market_div.attr("value")){
		...
		if(_orderType == "1" || _orderType == "2"){
			...
			service_credit.stock_link(param,function(data){
			  	if(isMax){
			  	  	queryMaxAmountCallBackMax(data);
		  	  	}else{
		  	  		queryMaxAmountCallBack(data);
	...
	function queryHolding(){
		...
		if(_orderType=="1" || _orderType=="2"){
			service_stock.queryStockData(param,queryHoldingCallback,{"isLastReq":true,"isShowWait":false,"isShowOverLay":false});
		}else{
			service_credit.cc_query(param,queryHoldingCallback,{"isLastReq":true,"isShowWait":false,"isShowOverLay":false});
		}
	}
	...
	function queryHoldingHTML_stock(element){
        ...
        eleHtml+="<dd><p><strong>"+element.stock_name +"</strong></p><p><small>"+Number(element.market_value).toFixed(price_digit) +"</small></p></dd>";
        eleHtml+="<dd><p><span class=\""+css+"\">"+float_yk+"</span></p><p><span class=\""+css+"\">"+Number(element.float_yk_per).toFixed(price_digit)+"%</span></p></dd>";
	...
	function clearBuyMsg(){
    	...
    	stock_html.stockName.text(""); // 股票名称
	...
	function orderPageBack(){
		var curPageCode = $.getCurrentPageObj().pageCode;
		var nextPageCode = "";
		if(_orderType=="1" || _orderType=="2"){
			nextPageCode = "account/index";
		}else{
			nextPageCode = "credit/order/mainOrder";
		}
		$.pageInit(curPageCode,nextPageCode,{});
	}
