 ...
 * 新股-申购
 */
define('trade/scripts/credit/shares/purchase.js',function(require, exports, module) {
	...
	var _pageId = "#credit_shares_purchase ";
    var userInfo =null;
    var num_step = 500; //步数
...
//		},"input");
	...
	function queryMessages(){
		...
		//首创的bus两融的查询新股列表的接口303053没有，只有普通交易的301535这个接口
//		service_credit.queryNewslist(param,messageCallBack);
	...
	function messageCallBack(data){
		if(data.error_no == 0){
			...
			if( results && results.length>0){
				var html = '<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ce_table"><tr><th scope="col">名称</th><th scope="col">发行价</th><th scope="col">每手(股)</th><th scope="col">申购上限(股)</th></tr>';
				...
				$.bindEvent($(_pageId+".fund_table .newShare"),function(e){
					...
					$(_pageId +"#stockUnit").val($(this).find("td").eq(3).find("span").text());
	...
	function messageAddHtml(element){
        ...
        eleHtml += '<td><strong>'+element.stock_name+'</strong><small>'+element.subscribe_code+'</small></td>';
        eleHtml += '<td><span>'+Number(element.issue_price).toFixed(3)+'</span></td>';
        eleHtml += '<td><span>'+Number(element.applyunitonline)+'</span></td>';
        eleHtml += '<td><span>'+Number(element.applymax_online)+'</span></td>';
        eleHtml += '</tr>';
		return eleHtml;
	}
	...
	function querynewAmount(fund_account,exchange_type){
		...
		service_credit.queryPurchaseLimit(param,queryNewAmountCallBack,{"isLastReq":true,"isShowWait":false});
	}
	//新股申购额度查询回调
