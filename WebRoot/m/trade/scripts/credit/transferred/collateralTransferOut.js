 ...
 * 担保品转出
 */
define('trade/scripts/credit/transferred/collateralTransferOut.js', function(require, exports, module) {
	var gconfig = $.config;
	var global = gconfig.global;
    var _pageId = "#credit_transferred_collateralTransferOut ";
...
//			$.pageInit("credit/transferred/collateralTransferOut","credit/transferred/collateralTransfer",{});
			$.pageBack("credit/transferred/collateralTransfer","left");
			e.stopPropagation();
		});
		//查询按钮
		$.bindEvent($(_pageId+".top_title .noicon_text"), function(e){
			$.pageInit("credit/transferred/collateralTransferOut","credit/transferred/transactionQuery",{});
			e.stopPropagation();
		});
	}
    ...
    function queryTradePosition(){
		 ...
		 * 担保信息查询(划出信息查询)
		 */
		service_credit.queryTransferPositions(param,queryMsgCallBack,
		{  //传给接口的json格式数据            
			"isLastReq":true,                
			"isShowWait":true,
			"isShowOverLay":false,
    ...
    function creatBoxHtml(codeInfo){
		...
		tipStr += '</div><div class="input_box"><label>转出数量</label>';
		tipStr += '<div class="input_value">';
    	tipStr += '<input type="tel" class="t1"/></div></div></div></div>';
		common.iConfirm("转出",tipStr,function($html){
    ...
    function getStockMsg(codeInfo){
		...
		param.entrust_bs = "1";//买卖方向（0：买入--普转信，1：卖出--信转普）
	...
	function stockTransIn(codeInfo,result)
	{
		...
		param.fund_account_crdt = userInfo.fund_account;//资金账户（信用）
		param.entrust_amount = codeInfo.entrust_amount;//过户数量
		param.stock_account_crdt = result.stock_account_crdt || userInfo.stock_account;//证券账号（信用）  || $.trim(codeInfo.stock_account)
