 ...
 * 港股交易-持仓
 */
define('trade/scripts/ggt/ggtHolding.js',function(require, exports, module) {
	...
	function bindPageEvent(){
		...
		$.bindEvent($(_pageId + ".top_title .icon_back"), function(e){
			$.pageInit("ggt/ggtHolding","account/index",{},true);
			e.stopPropagation();
		});
		//转账
		$.bindEvent($(_pageId + "#transfer"), function(e){
			$.setSStorageInfo("whichAccount","stock_userInfo");
			$.setSStorageInfo("whichPage","ggt/ggtHolding");
			$.pageInit("ggt/ggtHolding","banking/transfer",{});
			e.stopPropagation();
		},"tap");
	}
	...
	function queryData(){
		...
		if(global.positionRefreshTime!=0){
			position_timer = window.setInterval(queryPosition, global.positionRefreshTime);
		}
	}
	/**
	 * 资金账户查询
	...
	function queryCapital(){
		...
		//最后一个为超时处理函数
		service_ggt.queryFundSelect_ggt(param,queryCapitalCallback,
		{  //传给接口的json格式数据            
		"isLastReq":true,                
		"isShowWait":true,
		"isShowOverLay":false,
		...
		if (data && data!= "undefined"){
			if(data.error_no == 0){
				...
				if(results && results!= "undefined" && results.length >0){
							var assert_val = common.numToMoneyType(""+Number(results[0].assert_val).toFixed(2)+"");
							$(_pageId+".hold_main2 .inner>strong").html(assert_val.substring(0,assert_val.length-2)+"<em>"+ assert_val.substr(-2) +"</em>");//总资产
							var enable_balance = common.numToMoneyType(""+Number(results[0].enable_balance).toFixed(2)+"");
							$(_pageId+".hold_main2 .more_info li:eq(0)").find("strong").html(enable_balance.substring(0,enable_balance.length-2)+"<em>"+ enable_balance.substr(-2) +"</em>");//可取
							var fetch_cash = common.numToMoneyType(""+Number(results[0].fetch_cash).toFixed(2)+"");
							$(_pageId+".hold_main2 .more_info li:eq(2)").find("strong").html(fetch_cash.substring(0,fetch_cash.length-2)+"<em>"+ fetch_cash.substr(-2) +"</em>");//可用
							//总盈亏
//							var  = common.numToMoneyType(""+Number(results[0].enable_crash).toFixed(2)+"");
//							$(_pageId+".hold_main2 .more_info li:eq(2)").find("strong").html(enable_crash.substring(0,enable_crash.length-2)+"<em>"+ enable_crash.substr(-2) +"</em>");//可用
							
							//市值
							var market_val = common.numToMoneyType(""+Number(results[0].market_val).toFixed(2)+"");
...
//						}else if(results[i].money_type=="2"){
//							var assert_val = common.numToMoneyType(""+Number(results[i].assert_val).toFixed(2)+"");
//							$(_pageId+"#HKD .inner>strong").html(assert_val.substring(0,assert_val.length-2)+"<em>"+ assert_val.substr(-2) +"</em>");//总资产
//							var fetch_balance = common.numToMoneyType(""+Number(results[i].fetch_balance).toFixed(2)+"");
//							$(_pageId+"#HKD .more_info li:eq(1) strong").html(fetch_balance.substring(0,fetch_balance.length-2)+"<em>"+ fetch_balance.substr(-2) +"</em>");//可取
//							var enable_balance = common.numToMoneyType(""+Number(results[i].enable_balance).toFixed(2)+"");
//							$(_pageId+"#HKD .more_info li:eq(0) strong").html(enable_balance.substring(0,enable_balance.length-2)+"<em>"+ enable_balance.substr(-2) +"</em>");//可用
//						}else if(results[i].money_type=="1"){
//							var assert_val = common.numToMoneyType(""+Number(results[i].assert_val).toFixed(2)+"");
//							$(_pageId+"#Dollar .inner>strong").html(assert_val.substring(0,assert_val.length-2)+"<em>"+ assert_val.substr(-2) +"</em>");//总资产
//							var fetch_balance = common.numToMoneyType(""+Number(results[i].fetch_balance).toFixed(2)+"");
//							$(_pageId+"#Dollar .more_info li:eq(1) strong").html(fetch_balance.substring(0,fetch_balance.length-2)+"<em>"+ fetch_balance.substr(-2) +"</em>");//可取
//							var enable_balance = common.numToMoneyType(""+Number(results[i].enable_balance).toFixed(2)+"");
//							$(_pageId+"#Dollar .more_info li:eq(0) strong").html(enable_balance.substring(0,enable_balance.length-2)+"<em>"+ enable_balance.substr(-2) +"</em>");//可用
				}else{
	...
	function queryPosition(){
		...
		service_ggt.queryHolding_ggt(param,function(data){
			if (data && data != "undefined") {
				if(data.error_no == 0){
					var results = data.results;
					if(results && results.length>0){
						var html = "<tr><th scope=\"col\">证券/市值</th> <th scope=\"col\">盈亏</th><th scope=\"col\">持仓/可用</th><th scope=\"col\">成本/现价</th><th></th></tr>";
						float_yks = 0; //浮动盈亏
...
//						}
						...
						var fa = sign + common.numToMoneyType(""+float_yks);
						$(_pageId+".hold_main2 .more_info li:eq(1)").find("strong").html(fa.substring(0,fa.length-2)+"<em>"+ fa.substr(-2) +"</em>");//盈亏
//						//卖出事件
...
//								$(_pageId+".fund_table .ce_table tr").removeClass("active")
//								// $(_pageId+".fund_table .ce_table tr.tools").toggle();
...
//						},"tap");
						...
						//卖出事件
						$.bindEvent($(_pageId+'.fund_table .ce_table tr[class="part_option"]'), function(e){
	...
	function queryPositionHTML_stock(element){
		...
		eleHtml+="<tr id=\""+element.stock_code +"\" class='part_option'>";
		...
		eleHtml+="<td>"+Number(element.hold_amount)+"<br/>"+Number(element.enable_amount)+"</td>";
	...
	function destroy(){
	    ...
	    canBuy = 0 ;//可用
	    market_val = 0 ;//市值
		$(_pageId+".hold_main2 .inner>strong em").text("--");//总资产
		$(_pageId+".hold_main2 .more_info li strong em").text("--");
		if(myScroller)
		{
			myScroller.destroy();
			myScroller = null;
	 ...
	 */
	function pageBack(){
		$.pageInit("ggt/ggtHolding","account/index",{});
	}
