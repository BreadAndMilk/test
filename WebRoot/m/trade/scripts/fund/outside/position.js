 ...
 * 场外基金-基金持仓
 */
define('trade/scripts/fund/outside/position.js', function(require, exports, module) {
	var common = require("common");
	var gconfig = $.config; 
    var global = gconfig.global;
    var _pageId = "#fund_outside_position ";
		...
		if (data) {
			if(data.error_no == 0){
				...
				if(results && results.length > 0){
					var assert_val = Number(results[0].assert_val).toFixed(2)+"";
					$(_pageId+" .inner>strong").html(assert_val.substring(0,assert_val.length-2)+"<em>"+ assert_val.substr(-2) +"</em>");//总资产
					var fetch_balance = Number(results[0].fetch_balance).toFixed(2)+"";
					$(_pageId+" .more_info li").eq(2).find("strong").html(fetch_balance.substring(0,fetch_balance.length-2)+"<em>"+ fetch_balance.substr(-2) +"</em>");//可取
					var enable_balance = Number(results[0].enable_balance).toFixed(2)+"";
					$(_pageId+" .more_info li").eq(0).find("strong").html(enable_balance.substring(0,enable_balance.length-2)+"<em>"+ enable_balance.substr(-2) +"</em>");//可用
					...
					$(_pageId+" .more_info ul li").eq(1).find("strong").html(fas.substring(0,fas.length-2)+"<em>"+ fas.substr(-2) +"</em>").attr("class",css);//盈亏
					//添加当日浮动盈亏
					float_yks = results[0].today_yk || results[0].daily_income_balance;
					sign = "";
					css = "";
					//当浮动盈亏,浮动盈亏比例大于0 时,在前面添加一个 "+" 
					if(float_yks > 0){
					...
					var fa = sign + common.numToMoneyType(float_yks,2);
					$(_pageId+" .more_info ul li").eq(3).find("strong").html(fa.substring(0,fa.length-2)+"<em>"+ fa.substr(-2) +"</em>").attr("class",css);//盈亏
				}
				else{
	...
	function queryFundStockHTML(element){
        ...
        eleHtml += '<div class="title"><span class="stat">正常</span>';
        eleHtml += '<h5>'+element.fund_name+'<small>'+element.fund_code+'</small></h5></div><ul>';
        eleHtml += '<li>最新净值<span class="'+css+'">'+Number(element.nav)+'</span></li>';
        eleHtml += '<li>持有份额<span>'+Number(element.enable_shares)+'</span></li>';
        eleHtml += '<li>基金市值<span class="'+css+'">'+Number(element.market_value)+'</span></li>';
        eleHtml += '<li>分红方式<span>'+dividendmethod+'</span></li>';
        eleHtml += '</ul></div>';
        return eleHtml;
	}
	/**
