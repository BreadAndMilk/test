 ...
 * 分级基金-基金分拆
 */
define('trade/scripts/fund/grading/split.js', function(require, exports, module) {
	var gconfig = $.config;
	var global = gconfig.global;
	var common = require("common");
	var _pageId = "#fund_grading_split ";
	...
	function getFundList(fundStr){
		if(fundStr!= "" && fundStr.length==6){
			...
			service_fund.gradingSplitMergeQuery(param,getFundInfoBack);
		}	
	}
	function getFundInfoBack(data){
		if(data.error_no == 0){
			...
			if(results && results.length > 0){
				...
				$(_pageId+" .fund_split h5 em").text(results[0].stock_max_amount);//可用
			}
			else{
				$.alert("没有查询到对应的基金");
			}
	...
	function vailSubmitOrder(){
		...
		common.iConfirm("下单确定",tipStr,function success(){
			...
			var param = {
			    ...
			    service_fund.gradingSplitMerge(param,function(data){
...
//					queryCapital();
	...
	function clearBuyMsg(){
		$(_pageId+" .fund_split h5 em").text("--");//可用
